 ...
 * @author Firas
 */
public class ModifCommentaireController implements Initializable {
    ...
    @FXML
    private Button btn_modif_commentaire;
  ...
  try {
    ...
    ModifCommentaireController(int selectedCommentaireId) {
        this.commentaireId=selectedCommentaireId;
    }
    ...
    @FXML
    void click_modif(ActionEvent event) {
         // Vérifier si les champs sont vides
    if (text_contenu.getText().isEmpty() || text_id_user.getText().isEmpty() || text_like.getText().isEmpty() || text_dislike.getText().isEmpty()) {
    ...
    try {
        ...
        alert.setHeaderText(null);
        alert.setContentText("Les champs \"ID utilisateur\", \"J'aime\" et \"Je n'aime pas\" doivent contenir des valeurs numériques entières.");
        alert.showAndWait();
        return;
    }
    ...
    int id_post = Integer.parseInt(text_id_user.getText());
    int id_user = Integer.parseInt(text_id_user.getText());
    int like = Integer.parseInt(text_like.getText());
    int dislike = Integer.parseInt(text_dislike.getText());

    // Créer un objet Post à partir des données du formulaire
       Commentaire commentaire = new Commentaire(commentaireId, contenu, LocalDateTime.now(),id_post,id_user ,like, dislike);

    // Appeler la méthode modifierPost de CRUDPost pour mettre à jour le post dans la base de données
    try {
        CRUDCommentaire crudCommentaire = new CRUDCommentaire();
        crudCommentaire.modifierCommentaire(commentaire, commentaireId);
    } catch (SQLException e) {
        Alert alert = new Alert(Alert.AlertType.ERROR);
        alert.setTitle("Erreur");
        alert.setHeaderText(null);
        alert.setContentText("Une erreur est survenue lors de la mise à jour du post : " + e.getMessage());
        alert.showAndWait();
        return;
    }
